...
class DetThread(QThread):
    ...
    def __init__(self):
        ...
        self.weights = './detection/yolov5s.pt'
        self.current_weight = './detection/yolov5s.pt'
        self.source = '0'
        ...
        self.rate = 100
        self.save_fold = './result/detection'
        ...
        try:
            ...
            while True:
                ...
                if self.is_continue:
                    ...
                    self.send_img.emit(im0) #im0 : result  im0 [888,1574,3]  img[1,3,384,460]
                    self.send_raw.emit(im0s if isinstance(im0s, np.ndarray) else im0s[0]) #im0s : ori
                    ...
                    if percent == self.percent_length:
                        print(count)
                        self.send_percent.emit(0)
                        self.send_msg.emit('finished')  #出现在左下角的标志
                        if hasattr(self, 'out'):
                            self.out.release()
                        break
